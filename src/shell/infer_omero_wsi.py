...
def _tile_fetch_producer(
    conn: BlitzGateway,
    tile_schedule: list[tuple[int, int, int, int]],
    pixels_id: int,
    best_level: int,
    channel_ids: list[int],
    best_lsz_x: int,
    ...
    try:
        n = len(tile_schedule)
        for tile_idx, (oy0, ox0, oh, ow) in enumerate(tile_schedule):
            ...
            tile_eho = apply_eho_chunked(tile_rgb, **stain_params)
            del tile_rgb

            tile_queue.put((oy0, ox0, oh, ow, tile_eho))
...
def infer_omero_wsi(
    ...
    * The **main thread** pulls tiles from the queue and runs model
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # 5. Build tile schedule
        # ------------------------------------------------------------------
        tiles_total = 0
        tiles_tissue = 0
        tile_schedule: list[tuple[int, int, int, int]] = []

        for oy0 in range(0, out_h, inference_tile_size):
            oh = min(inference_tile_size, out_h - oy0)
            for ox0 in range(0, out_w, inference_tile_size):
                ow = min(inference_tile_size, out_w - ox0)
                tiles_total += 1
                if no_tissue_crop or _tile_has_tissue(
                    tissue_mask,
                    thumb_lsz_y,
                    thumb_lsz_x,
                    out_h,
                    ...
                    tile_schedule.append((oy0, ox0, oh, ow))
                    tiles_tissue += 1
...
def _run_pipeline(
    *,
    ...
    The fetch thread handles all OMERO I/O and EHO (numpy, releases GIL).
    The main thread handles only torch inference.  The two threads never
    share Ice proxies or torch objects, so they are safe on all platforms.
    """
    ...
    try:
        ...
        while True:
            ...
            oy0, ox0, oh, ow, tile_eho = item

            if eho_canvas is not None:
                eho_canvas[oy0 : oy0 + oh, ox0 : ox0 + ow] = tile_eho

            tile_pred = run_inference(tile_eho, model, device)
            del tile_eho

            ph, pw = tile_pred.shape[:2]
            copy_h = min(oh, ph)
            copy_w = min(ow, pw)
            pred[oy0 : oy0 + copy_h, ox0 : ox0 + copy_w] = tile_pred[:copy_h, :copy_w]
            del tile_pred
